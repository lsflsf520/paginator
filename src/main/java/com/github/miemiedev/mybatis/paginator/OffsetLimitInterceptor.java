...
public class OffsetLimitInterceptor implements Interceptor{
	...
	public Object intercept(final Invocation invocation) throws Throwable {
        ...
        if(pageBounds.isContainsTotalCount()){
            ...
            dataList = new PageList(listFuture.get(),countFutrue.get());
        }else{
        	dataList = listFuture.get();
        }

        if(!CollectionUtils.isEmpty(pageBounds.getOrders())){
        	if(!(dataList instanceof PageList)){
        		dataList = new PageList(dataList);
        	}
        	((PageList)dataList).addOrder(pageBounds.getOrders().toArray(new Order[0]));
        }
        return dataList;
	}
